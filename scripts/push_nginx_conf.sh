...
if [[ -z "$DRY_RUN" ]]; then
  ...
  ssh $SSH_OPTS "$REMOTE_USER_HOST" "mkdir -p $REMOTE_TMP/conf.d $REMOTE_TMP/ssl_files $REMOTE_TMP/sites-available"
  ...
  scp -r -p $SSH_OPTS "$SRC/sites-available/." "$REMOTE_USER_HOST:$REMOTE_TMP/sites-available/" 2>/dev/null || true

  # 2) 以 root 权限拷入 /etc/nginx 并校验+reload（nginx -t 不过则不会 reload）；末尾清理临时目录
  echo "==> 以 sudo 拷入 /etc/nginx 并校验配置"
  ssh $SSH_OPTS "$REMOTE_USER_HOST" "sudo cp -f $REMOTE_TMP/nginx.conf /etc/nginx/ && sudo cp -rf $REMOTE_TMP/conf.d/. /etc/nginx/conf.d/ && (sudo cp -rf $REMOTE_TMP/ssl_files/. /etc/nginx/ssl_files/ 2>/dev/null || true) && (sudo cp -rf $REMOTE_TMP/sites-available/. /etc/nginx/sites-available/ 2>/dev/null || true) && sudo nginx -t && sudo nginx -s reload; sudo rm -rf $REMOTE_TMP"
  echo "==> 完成"
else
