...
- 模板：`剧情配置/事件与节点配置表.xlsx`（**事件 / 剧情节点 两 sheet，第一行表头不读**）
- `scripts/gen-config-template.mjs` 生成模板（内部 `mkNode` 按表头映射，杜绝手写数组对不齐）
...
- **不填编号、不填 code**：编号/code 全由 `resolve-config.mjs` 从「中文名→父中文名」的关系树推出来。
- **中文名全局唯一**（重名直接报错）。
- **父中文名 = 自己爹的中文名**，只管爹是谁；顶级事件父中文名留空。
- 节点是事件/其他节点的子树，**自然继承前缀**：事件 `1` → 其节点 `1.1` → `1.1.1`。
...
4. **⚠️ 无视「强行当爹」**：任何按钮/战斗跳转造成的「额外父边」（即别的节点把某元素当终点）**不参与编号**，编号只看填写者声明的父。

示例（模板自带）：

| 中文名 | 父中文名 | 生成编号 | 生成 code |
|---|---|---|---|
| 初入青云 | （空） | `1` | `nb_1` |
| 青云内门 | （空） | `2` | `nb_2` |
| 山门之前 | 初入青云 | `1.1` | `n_11` |
| 叩门 | 山门之前 | `1.1.1` | `n_111` |
| 观望 | 山门之前 | `1.1.2` | `n_112` |
| 正厅 | 叩门 | `1.1.1.1` | `n_1111` |
| 内门入口 | 青云内门 | `2.1` | `n_21` |
| 内门深处 | 内门入口 | `2.1.1` | `n_211` |

> `正厅` 的父中文名只填了「叩门」，所以编号 `1.1.1.1`；即便 `观望`(1.1.2) 的按钮也指向 `正厅`（强行当爹），也**不改** `正厅` 的编号。同理 `内门深处` 回指 `山门之前` 只是普通跳转目标。

### 18.3 图片列（只填中文名，AI 补全路径）

- 填写者填纯中文文件名（如 `平儿.jpg`）；`fill-image-paths.mjs` 在 `图片/` 目录按**完全同名**递归查找，补全为真实 OSS 相对路径（如 `/cg/清纯/平儿.jpg`），已是带 `/` 的路径不动，找不到保留原名并告警。**不转拼音**。

### 18.4 按钮 / 战斗跳转目标填「目标中文名」

- 按钮与战斗的跳转目标填**目标元素的中文名**（不是编号）；`resolve-config.mjs` 查表翻译成 code，写入 `_已解析.xlsx` 的「目标code」列，查不到则在「校验」sheet 报错。
